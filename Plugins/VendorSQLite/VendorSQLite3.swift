...
struct VendorSQLite: CommandPlugin {
    ...
    func performCommand(context: PluginContext, arguments: [String]) async throws {
        ...
        // Find C target
        guard let target = try context.package.targets(named: ["NIOCSQLite"]).first.flatMap({ $0 as? ClangSourceModuleTarget }) else {
            throw VendoringError("Unable to find the NIOCSQLite target in package.")
        }
        if self.verbose { Diagnostics.progress("Found NIOCSQLite target with path \(target.directory)") }
